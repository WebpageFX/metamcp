...
<AccordionGroup>
  ...
  <Accordion icon="settings" title="配置环境">
    ...
    <Warning>在生产环境中，请确保修改 BETTER_AUTH_SECRET。同时使用 HTTPS。生成密钥的典型方法是 `openssl rand -hex 32 | base64`</Warning>
  </Accordion>
...
<AccordionGroup>
  <Accordion icon="warning" title="常见问题">
    ...
    **数据库连接**：检查 SQLite 卷已挂载，且 `DATABASE_URL` 指向可写路径。
