...
- **Docker**（可选，用于 Compose / Dev Containers）
- **Git** 用于版本控制
...
<AccordionGroup>
  ...
  <Accordion icon="cog" title="环境设置">
    ...
    根据需要修改 `.env.local` 文件以适配您的开发设置。
  </Accordion>

  <Accordion icon="database" title="数据库设置">
    MetaMCP 使用 SQLite。将 `example.env` 复制为 `.env.local` 后运行迁移：
...
<AccordionGroup>
  <Accordion icon="database" title="架构迁移">
    **创建迁移：**
    ...
    # 应用迁移
    pnpm db:migrate:dev # 使用 .env.local 中的 DATABASE_URL
