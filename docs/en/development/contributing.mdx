...
- **Docker** (optional, for Compose / Dev Containers)
...
<AccordionGroup>
  ...
  <Accordion icon="cog" title="Environment Setup">
    ...
    Modify the `.env.local` file as needed for your development setup.
  </Accordion>

  <Accordion icon="database" title="Database Setup">
    MetaMCP uses SQLite. After copying `example.env` to `.env.local`, apply migrations:
...
<AccordionGroup>
  <Accordion icon="database" title="Schema Migrations">
    **Creating migrations:**
    ...
    # Apply migrations
    pnpm db:migrate:dev # uses .env.local for DATABASE_URL
