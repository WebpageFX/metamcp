...
<AccordionGroup>
  ...
  <Accordion icon="settings" title="Configure Environment">
    ...
    <Warning>In production, make sure you modify `BETTER_AUTH_SECRET`. Also use HTTPS. A typical way to generate secrets is `openssl rand -hex 32 | base64`</Warning>
  </Accordion>
  ...
  <Accordion icon="docker" title="Start with Docker Compose">
    ...
    - SQLite database file (Compose volume)
...
<AccordionGroup>
  <Accordion icon="warning" title="Common Issues">
    ...
    **Database connection**: Check that the SQLite volume is mounted and `DATABASE_URL` points at a writable path.
