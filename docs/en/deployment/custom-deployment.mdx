...
```bash
cd /opt
sudo git clone https://github.com/fanywebfx/metamcp.git
...
**⚠️ IMPORTANT SECURITY NOTE**: Because this is a production environment, make sure you modify `BETTER_AUTH_SECRET` from its default value. Also ensure you use HTTPS. A typical way to generate secure secrets is:
...
# SQLite file inside the Compose volume (default: /data/metamcp.db)
